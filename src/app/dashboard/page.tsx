...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        if (preordersError) {
          // Do not hard fail dashboard if preorders fail; just continue with orders
          console.warn('Dashboard: Failed to load preorders', preordersError);
        }

        const ids = Array.from(
          new Set(
            [
              ...((ordersData || []).map(o => o.gamingpc_uuid).filter((v: unknown): v is number => typeof v === 'number')),
              ...((preordersData || []).map(p => p.gamingpc_uuid).filter((v: unknown): v is number => typeof v === 'number')),
            ]
          )
        )
  ...
  useEffect(() => {
    const ids = Array.from(
      new Set([
        ...((orders || []).map(o => o.gamingpc_uuid).filter((v: unknown): v is number => typeof v === 'number')),
        ...((preorders || []).map(p => p.gamingpc_uuid).filter((v: unknown): v is number => typeof v === 'number')),
      ])
    );
    ...
    <div className="min-h-screen">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mt-12">
          {preorders.length > 0 ? (
            <>
              <h3 className="text-lg font-bold text-gray-900 mb-4">Biðlisti</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                {preorders.map((po) => {
                  return (
                    <div key={po.id} className="relative h-full">
                      <div className="group relative h-full rounded-2xl border bg-white shadow-sm hover:shadow-md transition-shadow border-gray-200">
                        <div className="rounded-2xl p-5 h-full flex flex-col">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <div className="mt-2 flex items-center justify-start gap-2 text-sm text-gray-700 w-full">
                                {po.gamingpc_uuid ? (
                                  ...
                                  >
                                    {pcNamesById[po.gamingpc_uuid] || 'Vara'}
                                  </button>
                                ) : (
                                  <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-black/5 text-gray-900 text-xs font-medium">Vara</span>
                                )}
                              </div>
                            </div>
                            <div className="text-right" />
                          ...
                          {/* Created */}
                          <div className="mt-4 text-xs md:text-sm text-gray-600 text-center">Skráð: {formatDate(po.created_at)}</div>
