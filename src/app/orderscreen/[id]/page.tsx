...
export default function OrderScreenPage() {
  ...
  const screenIdParam = params.id as string;
  const [screenItem, setScreenItem] = useState<ScreenRow | null>(null);
  ...
  const [selection, setSelection] = useState<{ months: number; insured?: boolean; finalPrice?: number } | null>(null);
  ...
  useEffect(() => {
    try {
      if (typeof window !== 'undefined') {
        ...
        if (raw) {
          const parsed = JSON.parse(raw) as { months?: number; insured?: boolean; finalPrice?: number };
          ...
          setSelection({ months, insured, finalPrice });
        } else {
          setSelection({ months: 3 });
        }
      }
    } catch {
      setSelection({ months: 3 });
    ...
    const fetchScreen = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setScreenItem(data as ScreenRow);
        }
      } catch {
        setScreenItem(null);
      } finally {
  ...
  useEffect(() => {
    ...
    if (!session?.user) {
      router.push(`/auth?redirect=/orderscreen/${encodeURIComponent(screenIdParam)}`);
      return;
    }
    ...
    const loadProfile = async () => {
      try {
        ...
        console.error('OrderScreen: Error loading profile:', { message });
        setProfileError('Tókst ekki að sækja notandaupplýsingar');
      }
    };
    loadProfile();
  }, [authLoading, session, user, screenIdParam, router, userProfile]);
  ...
  const doSubmit = async () => {
    ...
    try {
      ...
      // Use finalPrice from productscreen page if available; fallback to discounted price
      ...
      if (!(typeof finalMonthlyPrice === 'number' && Number.isFinite(finalMonthlyPrice) && finalMonthlyPrice > 0)) {
        const baseDigits = (screenItem?.verd || '').toString().replace(/\D+/g, '');
      ...
      console.error('Error submitting screen order:', error);
      setSubmitStatus('error');
    } finally {
      ...
      <div className="min-h-screen bg-gray-50 py-12">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Þessi skjár er ekki til.</p>
          ...
          <div className="px-4 py-5 sm:p-6">
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-8">
              ...
              <div className="flex items-center justify-between mb-3 gap-3 md:gap-0">
                <h3 className="text-lg font-medium text-gray-900 truncate max-w-[65%] md:max-w-none">{screenItem.framleidandi}</h3>
                <p className="text-2xl font-bold text-[var(--color-secondary)] whitespace-nowrap">
                  {(() => {
                    ...
                    const digits = (screenItem.verd || '').replace(/\D+/g, '');
              ...
              <div className="space-y-1 text-sm text-gray-700">
                <p><span className="font-medium">Upplausn:</span> {screenItem.upplausn || '—'}</p>
                <p><span className="font-medium">Skjátegund:</span> {screenItem.skjataekni || '—'}</p>
                <p><span className="font-medium">Endurnýjunartíðni:</span> {screenItem.endurnyjunartidni || '—'}</p>
              </div>
            </div>
            ...
            {userProfile && (
              <div className="bg-gray-50 rounded-lg p-6 mb-8">
                ...
                <div className="mt-4">
                  <Link
                    href={`/notendaupplysingar?from=orderscreen`} 
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-8">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Leigutímabil</h2>
